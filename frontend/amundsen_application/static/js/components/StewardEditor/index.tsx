...
import { ResourceType, UpdateMethod, UpdateStewardPayload } from 'interfaces';
...
interface StewardAvatarLabelProps extends AvatarLabelProps {
  link?: string;
  isExternal?: boolean;
}
...
export interface StateFromProps {
  isLoading: boolean;
  itemProps: StewardItemProps;
}

export type StewardEditorProps = ComponentProps &
  DispatchFromProps &
  StateFromProps &
  EditableSectionChildProps;

export type StewardItemProps = { [id: string]: StewardAvatarLabelProps };

interface StewardEditorState {
  errorText: string | null;
  itemProps: StewardItemProps;
  tempItemProps: { [id: string]: AvatarLabelProps };
}

export class StewardEditor extends React.Component<
  StewardEditorProps,
  StewardEditorState
> {
  private inputRef: React.RefObject<HTMLInputElement>;

  public static defaultProps: Partial<StewardEditorProps> = {
    errorText: null,
    isLoading: false,
    itemProps: {},
  ...
  saveEdit = () => {
    ...
    const updateArray: UpdateStewardPayload[] = [];
  ...
  render() {
    ...
    const stewardList = hasItems ? (
      <ul className="component-list">
        {Object.keys(itemProps).map((key) => {
          const steward = itemProps[key];
          const avatarLabel = React.createElement(AvatarLabel, steward);

          let listItem;
          if (steward.link === undefined) {
            listItem = avatarLabel;
          } else if (steward.isExternal) {
            listItem = (
              <a
                href={steward.link}
                target="_blank"
