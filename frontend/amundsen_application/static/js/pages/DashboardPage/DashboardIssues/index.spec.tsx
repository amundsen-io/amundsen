...
describe('DashboardIssues', () => {
  const setup = (propOverrides?: Partial<DashboardIssueProps>) => {
    const props: DashboardIssueProps = {
      isLoading: false,
    ...
    const wrapper = shallow<DashboardIssues>(<DashboardIssues {...props} />);
  ...
  describe('render', () => {
    ...
    it('renders issues if they exist', () => {
      ...
      expect(wrapper.find('.dashboard-issue-link').text()).toEqual('issue_key');
      expect(wrapper.find('.issue-title-name').text()).toContain('title');
      expect(wrapper.find('.dashboard-issue-status').text()).toContain('Open');
      expect(wrapper.find('.dashboard-issue-priority').text()).toContain('P2');
    });
    ...
    it('renders no link to issues if no issues', () => {
      ...
      expect(wrapper.find('.dashboard-issue-more-issues').length).toEqual(0);
    });
    ...
    it('renders link if there are issues', () => {
      ...
      expect(wrapper.find('.dashboard-issue-more-issues').text()).toEqual('1 open');
    });
    ...
    it('renders open issue and closed issue links if the urls are set', () => {
      ...
      expect(wrapper.find('.dashboard-issue-more-issues').first().text()).toEqual(
        '1 open'
      );
      expect(wrapper.find('.dashboard-issue-more-issues').at(1).text()).toEqual(
        '0 closed'
      );
    });
  });
