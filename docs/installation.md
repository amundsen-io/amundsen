...
2. Clone [this repo](https://github.com/amundsen-io/amundsen) and its submodules by running:
   ...
   ```
3. Enter the cloned directory and run below. If it's your first time, you may want to proactively go through [troubleshooting](troubleshooting) steps, especially the first one related to heap memory for ElasticSearch and Docker engine memory allocation (leading to Docker error 137):
    ```bash
    # For Neo4j Backend
