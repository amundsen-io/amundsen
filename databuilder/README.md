...
- `extract_nested_columns` (defaults to `False`) - if `True` nested columns will be extracted. See example below.
  ...
  ```
   - If `extract_nested_columns` is `False` the output will be:
      - `field1`:
      - `field3`: `long`
   - If `extract_nested_columns` is `True` the output will be:
      - `field1.field2`: `long`
      - `field3`: `long`
