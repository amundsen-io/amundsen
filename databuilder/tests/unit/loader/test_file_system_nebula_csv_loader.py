...
class TestFsNebulaCSVLoader(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_load(self) -> None:
        ...
        expected_node_path = os.path.join(here, f'../resources/fs_nebula_csv_loader/{folder}/nodes')
        expected_nodes = self._get_csv_rows(expected_node_path, itemgetter('KEY'))
        actual_nodes = self._get_csv_rows(conf.get_string(FsNebulaCSVLoader.VERTEX_DIR_PATH),
                                          itemgetter('KEY'))
        self.assertEqual(expected_nodes, actual_nodes)

        expected_rel_path = os.path.join(here, f'../resources/fs_nebula_csv_loader/{folder}/relationships')
        expected_relations = self._get_csv_rows(expected_rel_path, itemgetter('START_KEY', 'END_KEY'))
        actual_relations = self._get_csv_rows(conf.get_string(FsNebulaCSVLoader.EDGE_DIR_PATH),
    ...
    def _make_conf(self, test_name: str) -> ConfigTree:
        prefix = '/var/tmp/TestFsNebulaCSVLoader'

        return ConfigFactory.from_dict({
            FsNebulaCSVLoader.VERTEX_DIR_PATH: f'{prefix}/{test_name}/{"nodes"}',
            FsNebulaCSVLoader.EDGE_DIR_PATH: f'{prefix}/{test_name}/{"relationships"}',
            FsNebulaCSVLoader.SHOULD_DELETE_CREATED_DIR: True
        })
