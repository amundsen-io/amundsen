...
class TestOracleMetadataExtractor(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_extraction_with_single_result(self) -> None:
        with patch.object(SQLAlchemyExtractor, '_get_connection') as mock_connection:
            ...
            table = {'schema': 'test_schema',
                     ...
                     'cluster':
                         self.conf[OracleMetadataExtractor.CLUSTER_KEY]
                     }
            ...
            expected = TableMetadata('oracle', 'MY_CLUSTER', 'test_schema', 'test_table', 'a table for testing',
...
class TestOracleMetadataExtractorWithWhereClause(unittest.TestCase):
    def setUp(self) -> None:
        logging.basicConfig(level=logging.INFO)
        ...
        config_dict = {
            OracleMetadataExtractor.WHERE_CLAUSE_SUFFIX_KEY: self.where_clause_suffix,
            f'extractor.sqlalchemy.{SQLAlchemyExtractor.CONN_STRING}': 'TEST_CONNECTION'
        }
        self.conf = ConfigFactory.from_dict(config_dict)
...
class TestOracleMetadataExtractorClusterKeyNoTableCatalog(unittest.TestCase):
...
class TestOracleMetadataExtractorNoClusterKeyNoTableCatalog(unittest.TestCase):
