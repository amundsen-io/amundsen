...
from databuilder.extractor.kafka_schema_registry_extractor import \
...
EXPECTED_TABLES: List[TableMetadata] = [
    ...
    TableMetadata(
        ...
        [
            ...
            ColumnMetadata(
                "options", None, "array<lvl2_record:struct<item1_lvl2:string,item2_lvl2:array<lvl3_record:struct<item1_lvl3:string,item2_lvl3:string>>>>", 2),
        ],
        False,
    ),
    TableMetadata(
        ...
        [
            ColumnMetadata("SquareId", " The id of the square that is part of the Milano GRID", "int", 0),
            ColumnMetadata("Polygon", None, "array<coordinates:struct<longitude:double,latitude:double>>", 1),
        ],
        False,
    ),
]
...
class TestKafkaSchemaRegistryExtractor(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.conf = ConfigFactory.from_dict(
            {KafkaSchemaRegistryExtractor.REGISTRY_URL_KEY: 'http://example.com'}
        )
        self.maxDiff = None

    @patch('databuilder.extractor.kafka_schema_registry_extractor.requests.Session')
    @patch('databuilder.extractor.kafka_schema_registry_extractor.KafkaSchemaRegistryExtractor._get_all_subjects')
    ...
    @patch('databuilder.extractor.kafka_schema_registry_extractor.requests.Session')
    @patch('databuilder.extractor.kafka_schema_registry_extractor.KafkaSchemaRegistryExtractor._get_subject')
    @patch('databuilder.extractor.kafka_schema_registry_extractor.KafkaSchemaRegistryExtractor._get_subject_max_version')
    @patch('databuilder.extractor.kafka_schema_registry_extractor.KafkaSchemaRegistryExtractor._get_all_subjects')
    def test_extraction_successfully(self,
                                     all_subjects: MagicMock,
    ...
    @patch('databuilder.extractor.kafka_schema_registry_extractor.requests.Session.get')
    @patch('databuilder.extractor.kafka_schema_registry_extractor.requests.Response.json')
    def test_get_latest_schema_version(self,
                                       response_json: MagicMock,
                                       session_get: MagicMock) -> None:
        """
