...
class BasePostgresColUsageExtractor(Extractor):
    """
    ...
    def init(self, conf: ConfigTree) -> None:
        conf = conf.with_fallback(BasePostgresColUsageExtractor.DEFAULT_CONFIG)
        self._cluster = conf.get_string(BasePostgresColUsageExtractor.CLUSTER_KEY)

        self._database = conf.get_string(BasePostgresColUsageExtractor.DATABASE_KEY, default='postgres')

        self.sql_stmt = self.get_sql_statement(
            use_catalog_as_cluster_name=conf.get_bool(BasePostgresColUsageExtractor.USE_CATALOG_AS_CLUSTER_NAME),
            where_clause_suffix=conf.get_string(BasePostgresColUsageExtractor.WHERE_CLAUSE_SUFFIX_KEY),
        )
    ...
    def extract(self) -> Union[ColumnReader, None]:
    ...
    def _get_extract_iter(self) -> Iterator[ColumnReader]:
        """
        ...
        for key, group in groupby(self._get_raw_extract_iter(), self._get_table_key):
            for row in group:
                yield ColumnReader(row['database'],
                                       row['cluster'],
                                       row['schema'],
                                       row['table_name'],
                                       row['user_email'],
