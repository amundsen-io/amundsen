...
class DatabricksSQLTextWidget:
    """
    A textbox in a Databricks SQL dashboard.
...
def get_text_widgets(
    widgets: Iterable[Dict[str, Any]]
) -> List[DatabricksSQLTextWidget]:
    """
    ...
    return [
        DatabricksSQLTextWidget(widget)
        for widget in widgets
        if "text" in widget and "visualization" not in widget
    ]


def get_visualization_widgets(
    widgets: Iterable[Dict[str, Any]]
) -> List[DatabricksSQLVisualizationWidget]:
    """
    From the raw set of widget data returned from the API, filter down
    ...
    return [
        DatabricksSQLVisualizationWidget(widget)
        for widget in widgets
...
def generate_dashboard_description(
    text_widgets: List[DatabricksSQLTextWidget],
    viz_widgets: List[DatabricksSQLVisualizationWidget],
) -> str:
    """
...
class DatabricksSQLPaginatedRestApiQuery(RestApiQuery):
    """
    Paginated Databricks SQL API queries
    """

    def __init__(self, **kwargs: Any) -> None:
        super(DatabricksSQLPaginatedRestApiQuery, self).__init__(**kwargs)
