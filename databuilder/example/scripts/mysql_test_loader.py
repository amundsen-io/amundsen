...
def connection_string():
    user = 'superset_user:IOBcEx^KI9rv'
    host = '172.28.244.24'
    port = '3306'
    db = 'data-superset'
    ## return "mysql://%s@%s:%s/%s" % (user, host, port, db)
    return "mysql+mysqldb://%s@%s:%s/%s" % (user, host, port, db)

def run_mysql_job():
    where_clause_suffix = textwrap.dedent("""
        where c.table_schema = 'data-superset'
    """)
    ...
    job_config = ConfigFactory.from_dict({
        ...
        f'publisher.neo4j.{neo4j_csv_publisher.JOB_PUBLISH_TAG}': 'mysql_test',  # should use unique tag here like {ds}
    })
...
def create_table_last_update_job():
    ...
    sql_last_update = textwrap.dedent("""
        SELECT 
            'mysql' AS db,
            TABLE_CATALOG AS 'cluster',
            TABLE_SCHEMA as 'schema',
            TABLE_NAME as 'table_name',
            MAX(UNIX_TIMESTAMP(UPDATE_TIME)) as last_updated_time_epoch
        FROM information_schema.TABLES
        WHERE TABLE_SCHEMA = 'data-superset' AND UPDATE_TIME IS NOT NULL 
    ...
    last_update_extractor = SQLAlchemyExtractor()
    csv_loader = FsNeo4jCSVLoader()
    task = DefaultTask(extractor=last_update_extractor, loader=csv_loader, transformer=NoopTransformer())
    ...
    job_config = ConfigFactory.from_dict({
        ...
        f'publisher.neo4j.{neo4j_csv_publisher.NEO4J_USER}': neo4j_user,
        f'publisher.neo4j.{neo4j_csv_publisher.NEO4J_PASSWORD}': neo4j_password,
        f'publisher.neo4j.{neo4j_csv_publisher.JOB_PUBLISH_TAG}':  'test_mysql_last_update'  # TO-DO unique tag must be added
    })
...
if __name__ == "__main__":
    ...
    last_update_job = create_table_last_update_job()
    last_update_job.launch()
