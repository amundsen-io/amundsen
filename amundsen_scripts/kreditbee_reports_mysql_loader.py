"""
Script to load data into Neo4j and Elasticsearch for kreditbee_reports schema.
"""
...
def connection_string():
    user = 'amundsen'
    password = ''
    host = 'kb-kbnbfc-derived-variable-db01.kb_kbnbfc.lan'
    port = '3306'
    db = 'kreditbee_reports'
    return "mysql+pymysql://%s:%s@%s:%s/%s" % (user, password, host, port, db)


def run_mysql_job():
    where_clause_suffix = textwrap.dedent("""
        where c.table_schema = 'kreditbee_reports'
    """)
...
if __name__ == "__main__":
    ...
    start_time = datetime.strptime(datetime.now(asia_kolkata).strftime('%Y-%m-%d %H:%M:%S'), '%Y-%m-%d %H:%M:%S')
    ...
    end_time = datetime.strptime(datetime.now(asia_kolkata).strftime('%Y-%m-%d %H:%M:%S'), '%Y-%m-%d %H:%M:%S')
